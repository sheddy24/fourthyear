...
export default function Profile() {
  ...
  const wtoken = getWorkerDetails(); // Get token containing userid

  useEffect(() => {
    // Fetch profile data when component mounts
    const fetchProfile = async () => {
      try {
        const response = await fetch("http://localhost:8000/profile/worker", {
          headers: {
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        "http://localhost:8000/profile/worker/edit",
        {
          method: "PUT",
          headers: {
  ...
  const handlePasswordChange = async (e) => {
    ...
    try {
      const response = await fetch(
        "http://localhost:8000/profile/worker/change-password",
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
